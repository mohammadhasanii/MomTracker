...
const MapComponent = () => {
  ...
  const targetLocation = [49.740646, 34.097525]; // مختصات هدف
  const [distance, setDistance] = useState(0);

  useEffect(() => {
    const fetchLatestLocation = async () => {
      try {
        const response = await axios.get('https://8nvkd5q1-3001.euw.devtunnels.ms/api/getLatestLocation');
    ...
    <LocationProvider>
      <BrowserRouter>
        <nav>
          ...
          <Link to="/send-location">Send Your Location</Link>
        </nav>
        <Routes>
